...
<br/>To ensure all MySQL requests would be using the app's credentials, a **DbAuth** class creates specified connector (**connect()**), cursor(**create_cursor()**), commit(**commit()** and execute request(**get_data()**)).
